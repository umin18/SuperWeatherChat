...
class APIParser {
    ...
    func parseWeatherInfo(data : [String:Any]) -> [Weather] {
        ...
        for day in dailyWeather {
            ...
            weeklyWeather.append(Weather(timezone: timezone, date: String(date1), time: String(td1), summary: summary1, icon: icon, temperatureHigh: tempHigh, temperatureLow: tempLow))
        }
