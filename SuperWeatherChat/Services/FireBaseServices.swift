...
class FireBaseServices {
    ...
    func addGoogleUser(user : UserModel, uid : String, completionHandler : @escaping completion) {
        let userDict = ["FirstName":user.fname,"Lastname":user.lname,"Email":user.email,"Address":user.address,"Phone":user.phone]
        self.reference.child("User").child(uid).setValue(userDict, withCompletionBlock: { (error2, ref) in
            completionHandler(error2)
        })
    }
    ...
    func getUser(completionHandler : @escaping (UserModel?)->Void) {
        ...
        self.reference.child("User").child(user!.uid).observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            self.getUserImg(id: user!.uid, completionHandler: { (data, error) in
                let use = UserModel(userID: user!.uid, fname: snap["FirstName"] as! String, lname: snap["Lastname"] as! String, email: user!.email!, address: snap["Address"] as! String , phone: snap["Phone"] as! String, password: nil, image: UIImage(data: data ?? Data()), latitude: 37.43344, longitude: -121.3323)
    ...
    func removePost(postId: String, id: String, completionHandler: @escaping completion) {
        let user = Auth.auth().currentUser
        if user?.uid != id {
            var errorTemp = NSError(domain:"Cannot delete post because it does not belong to current user", code: 2, userInfo:nil)
            completionHandler(errorTemp)
            return
        }
        self.reference.child("Post").child(postId).removeValue() { (error, ref) in
            completionHandler(error)
        }
    }
